...
<script>
    ...
    function editForm(url) {
        ...
        $.get(url)
            ...
            .fail((errors) => {
                Swal.fire('Error', 'Tidak dapat menampilkan data', 'error');
            });
    }
    ...
    function deleteSelected(url) {
        if ($('input:checked').length > 1) {
            ...
            Swal.fire('Error', 'Pilih data yang akan dihapus', 'error');
        }
    }

    function cetakBarcode(url) {
        if ($('input:checked').length < 1) {
            Swal.fire('Error', 'Pilih data yang akan dicetak', 'error');
        } else if ($('input:checked').length < 3) {
            Swal.fire('Error', 'Pilih minimal 3 data untuk dicetak', 'error');
        } else {
            $('.form-produk')
                .attr('target', '_blank')
                .attr('action', url)
